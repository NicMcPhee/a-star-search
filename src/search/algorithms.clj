    ...
    (loop [max-states max-states
      ...
      (if (or (empty? frontier)
        ...
        (let [current (first (peek frontier))
              ...
              scored-children (map (fn[next-state] (+ current-cost (heuristic-fn current next-state))) current-children)
              child-score-map (zipmap current-children scored-children)]
          (recur (- max-states (count scored-children))
                 "next frontier / pick highest score child"
                 (reduce #(conj %1 %2) frontier child-score-map)
                 "updated came-from"
                 (into came-from (map #([%1 current]) current-children))
                 "updated cost-so-far"
                 (concat cost-so-far child-score-map))
          )
        )
      )
    )